...
import uuid
from typing import Dict, List, Optional, AsyncGenerator # Import AsyncGenerator
import time
...
class AppState:
    def __init__(self):
        ...
        if self.device == "cuda":
            try:
                 ...
                 print(f"Error setting GPU device: {e}. Falling back to default.")
                 self.device = "cpu"
                 self.llm_model.to(self.device)
        else:
            print("Using CPU")
        ...
        if USE_DOCS:
            if os.path.exists("doc_embeddings.pt"):
                self.doc_embeddings = torch.load("doc_embeddings.pt").to(self.device)
            else:
        ...
        self.background_task: Optional[asyncio.Task] = None # Store task handle here
    ...
    async def batched_retrieve_top_k(self, query_embeddings: torch.Tensor, k: int = 2) -> List[List[str]]:
        ...
        if USE_ANN:
            print("Using ANN")
            from .task_1.ann import ann
            indices, _ = ann(query_embeddings_tensor, self.doc_embeddings, k=actual_k)
        _, indices = torch.topk(sims, actual_k, dim=1)
        indices_np = indices.cpu().numpy()
        documents_np = np.array(self.documents)
        selected_docs_batch = [documents_np[indices_np[i]].tolist() for i in range(indices_np.shape[0])]
    ...
    def _blocking_generate_text_batch(self, prompts: List[str]) -> List[str]:
        ...
        print(f"LLM generation took {end_time - start_time:.4f} seconds for {len(prompts)} prompts.")
    ...
    async def batched_rag_pipeline(self, queries: List[str], k: int) -> List[str]:
        ...
        print(f"  Embeddings computed in {embed_time - pipeline_start_time:.4f}s")
        ...
        print(f"  LLM Generation finished in {generate_end_time - format_time:.4f}s")
        print(f"RAG Pipeline: Batch total time {generate_end_time - pipeline_start_time:.4f}s")
        return generated_texts
...
# --- Background Task ---
# ... (process_requests_loop function remains the same as the corrected one) ...
async def process_requests_loop(app_state: AppState):
    ...
    while True:
        ...
        try:
            ...
            print(f"Waiting for first item... Queue size: {app_state.request_queue.qsize()}")
            ...
            while len(batch) < MAX_BATCH_SIZE:
                ...
                try:
                    ...
                    # print(f"Added item to batch: {next_item._id}. Batch size: {len(batch)}")
        ...
        if batch:
            ...
            try:
                ...
                for i, req in enumerate(batch):
                    ...
                    if response_queue:
                        try: await response_queue.put(results[i])
                        except Exception as put_e: print(f"Error putting result for {req._id}: {put_e}")
                    else: print(f"Warning: Response queue not found for request {req._id}")
                batch_process_end_time = time.monotonic()
                print(f"Batch processed in {batch_process_end_time - batch_process_start_time:.4f}s")
                ...
                for req in batch:
                    response_queue = app_state.responses.get(req._id)
                    if response_queue:
                         try: await response_queue.put(error_message)
                         except Exception as notify_e: print(f"Error notifying client {req._id} about batch failure: {notify_e}")
        else:
...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    app_state.background_task = asyncio.create_task(process_requests_loop(app_state))
...
async def predict(payload: QueryRequest, request: Request):
    ...
    # print(f"Received request {payload._id} for query: '{payload.query[:30]}...'")

    try:
        await app_state.request_queue.put(payload)
        # print(f"Request {payload._id} queued. Queue size: {app_state.request_queue.qsize()}")
    except Exception as e:
        # print(f"Error queueing request {payload._id}: {e}")
        if payload._id in app_state.responses: del app_state.responses[payload._id]
        return {"error": "Failed to queue request"}

    try:
        # print(f"Waiting for response for {payload._id}...")
        ...
        # print(f"Response received for {payload._id} after {response_received_time - request_received_time:.4f}s")
        response_q.task_done()
        return {"response": response}
    except AsyncTimeoutError:
        # print(f"Timeout waiting for response for request {payload._id}")
        return {"error": "Processing timeout"}
    except Exception as e:
        # print(f"Error waiting for / processing response for {payload._id}: {e}")
        return {"error": f"An internal error occurred"}
    finally:
        if payload._id in app_state.responses:
            del app_state.responses[payload._id]
            # print(f"Cleaned up response queue for {payload._id}")
